...
namespace CtiDigital\Configurator\Component\Websites;
...
/**
 * Processor for the Websites component.
 *
 * Handles creation and updating of websites, store groups, and store views.
 *
 * Note: Magento's Store API repository interfaces (WebsiteRepositoryInterface,
 * StoreRepositoryInterface, GroupRepositoryInterface) are read-only — they do not
 * expose save() or create() operations. Resource-model saves are therefore retained
 * here by design and are not a service-contract violation.
 */
class WebsitesProcessor implements ComponentProcessorInterface
{
    private array $data = [];
    ...
    /**
     * No configuration block for the Websites processor.
     *
     * @return $this
     */
    ...
    /**
     * Process all websites, store groups, and store views from the source data.
     */
    public function process(): void
    {
        try {
            ...
            foreach ($this->data['websites'] as $code => $websiteData) {
                // Process the website
                ...
                foreach ($websiteData['store_groups'] as $storeGroupData) {
                    ...
                    // As the store may not be created yet, associating the default store to the store group
